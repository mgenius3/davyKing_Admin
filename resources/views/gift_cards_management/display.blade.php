...
@section('content')
    ...
    <div class="mb-md-3 mb-2 d-flex flex-wrap">
        ...
        <div class="me-4 mb-md-0 mb-2">
            <a href="#" class="text-decoration-none text-body d-flex align-items-center" data-bs-toggle="modal" data-bs-target="#addGiftCardModal">
    ...
    <div class="row gx-4">
        <div class="col-xl-8">
            ...
            <div class="card mb-4">
                <div class="card-header d-flex align-items-center bg-none fw-semibold">
                    ...
                    <a href="#" class="ms-auto text-decoration-none text-body text-opacity-50 fs-13px d-flex align-items-center" data-bs-toggle="modal" data-bs-target="#addGiftCardModal">
                    ...
                    @foreach ($giftCards as $giftCard)
                        <div class="row align-items-center">
                            ...
                            <div class="col-lg-3 m-0 ps-lg-3">
                                Buy: ${{ $giftCard->buy_rate }} | Sell: ${{ $giftCard->sell_rate }}
                            </div>
                            <div class="col-lg-4 m-0 text-end">
                                <button class="btn btn-sm {{ $giftCard->is_enabled ? 'btn-success' : 'btn-danger' }} toggle-status" 
                                        data-id="{{ $giftCard->id }}" 
                                        data-enabled="{{ $giftCard->is_enabled ? '1' : '0' }}"
                                        data-bs-toggle="tooltip" 
                                        title="Toggle {{ $giftCard->name }} status">
                                    {{ $giftCard->is_enabled ? 'Enabled' : 'Disabled' }}
                                </button>
                                <a href="#" class="btn btn-sm btn-outline-primary ms-2" data-bs-toggle="modal" data-bs-target="#editGiftCardModal{{ $giftCard->id }}">Edit</a>
                            </div>
                        </div>
                        @if (!$loop->last)
                        ...
                        <div class="modal fade" id="editGiftCardModal{{ $giftCard->id }}" tabindex="-1" aria-labelledby="editGiftCardModalLabel{{ $giftCard->id }}" aria-hidden="true">
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h5 class="modal-title" id="editGiftCardModalLabel{{ $giftCard->id }}">Edit {{ $giftCard->name }}</h5>
                                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                    </div>
                                    <form action="{{ route('admin.gift-cards.update', $giftCard->id) }}" method="POST" enctype="multipart/form-data">
                                        @csrf
                                        @method('PUT')
                                        <div class="modal-body">
                                            <div class="mb-3">
                                                <label class="form-label">Name</label>
                                                <input type="text" class="form-control" name="name" value="{{ $giftCard->name }}" required>
                                            </div>
                                            <div class="mb-3">
                                                <label class="form-label">Category</label>
                                                <input type="text" class="form-control" name="category" value="{{ $giftCard->category }}" required>
                                            </div>
                                            <div class="mb-3">
                                                <label class="form-label">Denomination</label>
                                                <input type="number" step="0.01" class="form-control" name="denomination" value="{{ $giftCard->denomination }}" required>
                                            </div>
                                            <div class="mb-3">
                        ...
                        <tbody>
                            @foreach ($transactions as $transaction)
                                <tr>
                                    ...
                                    <td><a href="{{ route('admin.gift-cards.transaction', $transaction->id) }}" class="text-decoration-none">{{ $transaction->giftCard->name }}</a></td>
        ...
        <div class="col-xl-4">
            ...
            <div class="card mb-4">
                <div class="card-header d-flex align-items-center bg-none fw-semibold">
                    Rate Management
                    <a href="#" class="ms-auto text-decoration-none fs-13px text-body text-opacity-50" id="refreshRates">Refresh</a>
                </div>
                <div class="card-body">
                    <form action="{{ route('admin.gift-cards.update-rates') }}" method="POST" id="rateForm">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select" name="gift_card_id" id="giftCardSelect">
                                @forelse ($giftCards as $giftCard)
                                    <option value="{{ $giftCard->id }}" data-buy-rate="{{ $giftCard->buy_rate }}" data-sell-rate="{{ $giftCard->sell_rate }}">{{ $giftCard->name }} ({{ $giftCard->denomination }})</option>
                                @empty
                        ...
                        <div class="mb-3">
                            <label class="form-label">Buy Rate <small>(Current: <span id="currentBuyRate">{{ $giftCards->isNotEmpty() ? $giftCards->first()->buy_rate : 'N/A' }}</span>)</small></label>
                            <input type="number" step="0.01" class="form-control" name="buy_rate" id="buyRate" placeholder="e.g., 0.80" {{ $giftCards->isEmpty() ? 'disabled' : '' }} required>
                        </div>
                        <div class="mb-3">
                            <label class="form-label">Sell Rate <small>(Current: <span id="currentSellRate">{{ $giftCards->isNotEmpty() ? $giftCards->first()->sell_rate : 'N/A' }}</span>)</small></label>
                            <input type="number" step="0.01" class="form-control" name="sell_rate" id="sellRate" placeholder="e.g., 0.95" {{ $giftCards->isEmpty() ? 'disabled' : '' }} required>
                        </div>
                        <button type="submit" class="btn btn-theme" data-bs-toggle="tooltip" title="Update rates for selected gift card" {{ $giftCards->isEmpty() ? 'disabled' : '' }}>Update Rates</button>
                    </form>
                </div>
            </div>
            ...
            <div class="modal-content">
                ...
                <form action="{{ route('admin.gift-cards.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            <label class="form-label">Denomination</label>
                            <input type="number" step="0.01" class="form-control" name="denomination" required>
                        </div>
                        <div class="mb-3">
